...
class Augment:
    ...
    def _random_color_jitter(self, x, p=.8):
        x = tf.cond(
            tf.less(tf.random.uniform([]), p),
            lambda: self._color_jitter(x),
            lambda: x)
        return x
    ...
    def _random_grayscale(self, x, p=.2):
        def _func():
            gray_x = x
            gray_x = self._grayscale(gray_x)
            gray_x = tf.tile(gray_x, [1, 1, 1, 3])
            return gray_x
    ...
    def _random_gaussian_blur(self, x, p=.5):
        def _func():
            ...
            blurred_x = tf.nn.depthwise_conv2d(blurred_x, gaussian_kernel_2d, (1, 1, 1, 1), "SAME")
            return blurred_x
